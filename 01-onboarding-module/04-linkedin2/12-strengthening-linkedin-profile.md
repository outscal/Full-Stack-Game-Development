# **Strengthening LinkedIn profile**



## 📝 Task:


---



In your LinkedIn profile page:

- Add experience if any.
- Add Education.
- Add GitHub Link (if any).
- Add your latest Resume.
- Add Portfolio link (if any).



## ✅ Submission:


---

- Submit the link to your LinkedIn profile as a submission for this assignment.
- Share your profile link in the **#build-in-public** channel of discord.
- Tell the community about your updated profile.



![image](//outscal.github.io/Full-Stack-Game-Development/images/1255772f50e61080.png)
