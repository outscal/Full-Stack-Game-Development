Before starting the Chapter, you need to clean up your main files as well!

Currently, your main file has `Professor Oak` class and also `gameloop()` method.

You just need to create a new .h and .cpp file for `Professor Oak` and `Game` classes and move the code accordingly.



> **TODO:**
> **✅**Create `ProfessorOak.h` and `ProfessorOak.cpp`
> ✅Create `Game.h` and `Game.cpp`
> ✅Move `gameloop()` method to `Game` class.
> ✅Move `ProfessorOak` class to `ProfessorOak.h` and `ProfessorOak.cpp`
> ✅Update the `Main.cpp` file.
...
Your are now ready to start the next lesson!💪🏼
