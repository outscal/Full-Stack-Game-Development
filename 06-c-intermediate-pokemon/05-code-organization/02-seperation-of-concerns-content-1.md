...
You realized that keeping everything in one file makes managing Your code quite chaotic.



You need a better way to organize Your code to make it clearer and more manageable.

This is where the principle of** Separation of Concerns (SoC)** comes in. 

SoC helps us break down our code into distinct sections, each focusing on a different aspect of the game.
...
Before breaking down Your code.

let’s take a closer look at it. By carefully examining what we have.

we can identify which parts can be separated into different files and which parts should stay together.



Click Here ( code till now )```cpp
#include <iostream>
...
You start with a single file containing all our code—functions, classes, and the game loop.

While this might work for a small project.

It can quickly become overwhelming as your codebase grows.



Let's understand your current setup:

You have a `Player` class that manages player-specific details.

You have a `Pokemon` class handling Pokémon attributes and behaviors.

You have functions like `clearConsole()` and `waitForEnter()`.

You have a `ProfessorOak` class that interacts with the player and provides game-related guidance.

You have the `gameLoop` function controlling the main flow of the game.



> 💡**PRO TIP:**
> Scroll on your Game Code repeatedly to understand the code organization chapter.
...
## 1. Separating the Player Class



Your `Player` class is responsible for player-specific details and actions. 

It makes sense to move this class into its seperate file to keep it organized. 



          You can create `Player.h` file for the class declaration and `Player.cpp` file for the class definition.



> **Note **: we will discuss about declaration and definition in next chapter.



Just Imagine you seperate out some code from a large piece of one code.

This separation makes it easier to manage and modify the player-related code without sifting through the entire file.



## 2. Separating the Pokemon Class



Next, You have the `Pokemon` class, which handles Pokémon-related attributes and methods. 

Just like with the `Player` class, we’ll create `Pokemon.h` and `Pokemon.cpp` to encapsulate the Pokémon functionality. 

This separation helps in isolating Pokémon-related code, making it easier to update or debug.
...
These functions can be grouped into a separate file dedicated to utility functions. 

This way, all utility functions are in one place, making them easy to find and manage.



> **Note **: We discuss about Utility Functions in later in this content.
...
The main game loop will be kept in `main.cpp`. 

This file will coordinate between the different parts of our code, managing the flow of the game.
...
![confusedoak](https://outscal-assets.s3.ap-south-1.amazonaws.com/production/LMS/634ff2a63bfd3eecf3cfe038/08_30_2024__08_58_50.png)
...
**Hold Your Horses, Professor Oak!**

Before you get too worked up, Professor, let’s not rush things. 

We’re just about to cover that very topic in the next lesson!
