...
- Make sure you are in the  `Feature-3_Fetch-Player-Input`  branch in your git repo.
- Make changes for this assignment (Add new commits) on this branch.
- Create a `Pull Request`, from this feature branch `Feature-3_Fetch-Player-Input` to the previous branch `main` if not already created.
...
1. Create a new folder: `Header/Event` and `Source/Event` 
2. Create `EventManager.h` in `Header/Event` 
3. Create `EventManager.cpp` in `Source/Event`
...
1. Implement `pollEvents(RenderWindow* game_window)` function. 
2. Implement `isKeyPressed(sf::Keyboard::Key key)` function.
...
1. Include `EventManager.h` in `main.cpp `
2. Add event polling to `main()` function. 
