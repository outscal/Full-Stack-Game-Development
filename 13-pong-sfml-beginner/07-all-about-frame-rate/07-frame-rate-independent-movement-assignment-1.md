...
1. Update `GameplayManager` with `TimeService` 
2. Modify Paddle movement to use delta time 
3. Modify Ball movement to use delta time 
...
1. Add `float elapsed_delay_time` and `float delay_duration` 
2. Add `void updateDelayTime(float delta_time)` method. 
3. Add a `BallState` enum with `Idle` and `Moving` states.

In `Ball.cpp` 

1. Add the logic for initial delay.
