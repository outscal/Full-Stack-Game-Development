...
- Make sure you are in the `Feature-7_Frame-Rate` branch in your git repo.
- Make changes for this assignment (Add new commits) on this branch.
- Create a `Pull Request`, from this feature branch `Feature-7_Frame-Rate` to the previous branch `main` if not already created.
...
1. In `Header/Utility` create `TimeService.h` 
2. Create all the function and variable declarations.
3. In `Source/Utility` create `TimeService.cpp`
...
1. Implement `void initialize()`
2. Implement `float calculateDeltaTime()`
3. Implement `void updateDeltaTime()`
4. Implement `void TimeService::updatePreviousTime()`
5. Implement `void update()`
6. Implement `float getDeltaTime()`
