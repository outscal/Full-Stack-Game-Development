...
- Pass the unit command form `**GameService**` to `**CommandInvoker**` through `**PlayerService**` 
- The command should pass through the player service, player controller & unit controller, allowing each component to add necessary data to it.
- `**UnitController**` should ask the `**CommandInvoker**` to process and execute the unit command when needed.
