...
1. In the `Board.h` file add the `openCell` method. 
2. Implement the `onCellButtonClicked` method to the `Board` class.
3. Add the `canOpenCell` method to the `Board` class. 
4. Implement the `openCell` method in the `Board` class. 
5. Add the `open` method to the `Cell` class. 
