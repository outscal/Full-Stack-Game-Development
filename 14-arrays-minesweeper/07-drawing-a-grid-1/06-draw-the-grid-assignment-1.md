...
- Make sure you are in the  `Feature-4_2D-Array`  branch in your git repo.
- Make changes mentioned in the assignment (Add new commits) on this branch.
- Create a `Pull Request`, from this feature branch `Feature-4_2D-Array` to the previous branch `main` if not already created.
...
- Since this is a guided assignment, code reviews will not happen, instead, the solution code is present in the assignment.
  [**Click Here To See The Solution Code**](https://github.com/outscal/Arrays/tree/Feature-4_2D-Array/Array-Minesweeper)
- But, before checking the solution code, try your best to solve it independently.



> 💡**PROTIP:**
> You can spoil yourself by looking at the [**solution**](https://github.com/outscal/Arrays/tree/Feature-4_2D-Array/Array-Minesweeper) before trying out the assignments, but we don't recommend learning that way.
...
1. Change the `Cell* board[numberOfColumns]` to `Cell* board[numberOfRows][numberOfColumns]` 



In `Board.cpp` 

1. Update the `createBoard()` method. 
